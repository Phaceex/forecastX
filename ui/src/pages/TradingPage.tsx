...
const TradingPage: React.FC = () => {
  ...
  const fixture = WORLD_CUP_FIXTURES.find(f => f.id === Number(fixtureId));
  ...
  const refreshState = useCallback(async () => {
    if (!pdas) return;
    try {
      const info = await connection.getAccountInfo(pdas.market);
      if (info && info.data.length >= 20) {
        setMarketExists(true);
        setMarketResolved(info.data[18] === 1);
        setWinningOutcome(info.data[19]);
      } else { setMarketExists(false); }
    } catch { setMarketExists(false); }

    try {
      const yr = await connection.getTokenAccountBalance(pdas.yesReserve);
      const nr = await connection.getTokenAccountBalance(pdas.noReserve);
      const yAmt = Number(yr.value.uiAmount) || 0;
      const nAmt = Number(nr.value.uiAmount) || 0;
      const total = yAmt + nAmt;
      setLiquidity(total);
      if (total > 0) { setYesPrice(nAmt/total); setNoPrice(yAmt/total); }
    } catch {}
  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      const yesAta = getAssociatedTokenAddressSync(pdas.yesMint, publicKey);
      const noAta = getAssociatedTokenAddressSync(pdas.noMint, publicKey);
      const redeemBal = winningOutcome === 1 ? yesBalance : noBalance;
      const redeemAta = winningOutcome === 1 ? yesAta : noAta;
  ...
  const estOutput = amount ? (Number(amount) * (tab.includes('yes') ? yesPrice : noPrice) * (tab.startsWith('sell') ? 1/yesPrice : 1)).toFixed(4) : '—';
        ...
        <div className="tp-left">
          ...
          <div className="tp-price-cards">
            ...
            <div className="tp-price-card stats">
              ...
              <div className="tppc-val">${liquidity > 0 ? liquidity.toFixed(2) : '—'}</div>
          ...
          <div className="tp-market-status glass-card">
            <div className="tpms-row">
              ...
              <span className={`tag ${marketExists ? (marketResolved ? 'tag-gray' : 'tag-green') : 'tag-red'}`}>
                {marketExists ? (marketResolved ? 'Resolved' : 'Active') : 'Not Listed'}
              </span>
            </div>
            {marketResolved && (
              <div className="tpms-row">
                <span className="tpms-label">Winner</span>
                <span className={`tag ${winningOutcome===1?'tag-green':'tag-red'}`}>
                  {winningOutcome===1 ? 'YES wins' : 'NO wins'}
                </span>
              </div>
            )}
            <div className="tpms-row">
        ...
        <div className="tp-right">
          ...
          <motion.div initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} transition={{delay:0.15}} className="tp-trade-box glass-card">
            <div className="tp-trade-title">
              {!marketExists ? 'List Market' : marketResolved ? 'Redeem Winnings' : 'Place Order'}
            </div>
            ...
            {!connected ? (
              ...
              <div className="tp-redeem">
                <div className={`tp-winner-banner ${winningOutcome===1?'yes':'no'}`}>
                  {winningOutcome===1 ? '🎉 YES wins!' : '🎉 NO wins!'}
                  <span>Redeem your {winningOutcome===1?'YES':'NO'} tokens for USDC</span>
                </div>
                <div className="tpb-item"><span>Redeemable tokens</span><span className="tpb-val">{winningOutcome===1 ? yesBalance.toFixed(4) : noBalance.toFixed(4)}</span></div>
                <button className="btn btn-success btn-lg" style={{width:'100%',marginTop:12}} onClick={handleRedeem} disabled={loading || (winningOutcome===1 ? yesBalance===0 : noBalance===0)}>
          ...
          <div className="tp-orderbook glass-card">
            ...
            {[...Array(5)].map((_, i) => {
              const p = (yesPrice * 100 + (i + 1) * 1.2).toFixed(1);
            ...
            <div className="tpob-spread">
              <span>Spread</span>
              <span>{(Math.abs(yesPrice - noPrice) * 100).toFixed(2)}¢</span>
            </div>
            {[...Array(5)].map((_, i) => {
              const p = (yesPrice * 100 - (i + 1) * 1.2).toFixed(1);
              const sz = (Math.random() * 200 + 50).toFixed(0);
